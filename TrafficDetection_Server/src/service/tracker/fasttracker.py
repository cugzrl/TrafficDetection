...
class STrack(BaseTrack):
    ...
    def re_activate(self, new_track, frame_id, new_id=False, track_id=None):
        ...
        self.classid = new_track.classid
...
class Fasttracker(object):
    def __init__(self, config, frame_rate=30):
        ...
        self.reset_velocity_offset_occ = config.get("reset_velocity_offset_occ", 3)
        self.reset_pos_offset_occ = config.get("reset_pos_offset_occ", 2)
        self.enlarge_bbox_occ = config.get("enlarge_bbox_occ", 1.1)
        self.dampen_motion_occ = config.get("dampen_motion_occ", 0.85)
        self.active_occ_to_lost_thresh = config.get("active_occ_to_lost_thresh", 15)
        self.init_iou_suppress = config.get("init_iou_suppress", 0.8)
        self.kalman_filter = KalmanFilter()

    # 新增函数
    def _next_track_id(self):
        """为当前 tracker 实例生成独立轨迹编号，避免跨请求污染。"""
        self._track_count += 1
        return self._track_count

    def update(self, output_results, img_info, img_size):
        """接收 Nx7 检测矩阵并输出当前帧活跃轨迹。"""
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        if output_results.shape[1] == 5:
            ...
            classid = np.full((output_results.shape[0],), -1, dtype=np.int32)
        else:
            scores = output_results[:, 4] * output_results[:, 5]
            bboxes = output_results[:, :4]  # x1y1x2y2
            classid = output_results[:, 6].astype(np.int32)
        ...
        classid_keep = classid[remain_inds]
        classid_second = classid[inds_second]
        ...
        if len(dets) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for
                          (tlbr, s, c) in zip(dets, scores_keep, classid_keep)]
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for
                          (tlbr, s, c) in zip(dets_second, scores_second, classid_second)]
        else:
            detections_second = []
